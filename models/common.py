...
class MHSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("content_position shape",content_position.shape)
        content_position = content_position if (content_content.shape == content_position.shape) else content_position[
                                                                                                      :, :, :c3, ]
        # print("new content_position shape",content_position.shape)
        assert (content_content.shape == content_position.shape)
        # print('new pos-> shape:',content_position.shape)
        # print('new content-> shape:',content_content.shape)
...
class LCA(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5, e2=1, w=20, h=20):  # ch_in, ch_out, number, , expansion,w,h
        ...
        c_ = int(c2 * e)  # hidden channels
        # dict_heads = dict([(64, 1), (128, 2), (256, 4), (512, 8), (1024, 16)])
        dict_w = dict([(64, 160), (128, 80), (256, 40), (512, 20)])
        dict_h = dict([(64, 160), (128, 80), (256, 40), (512, 20)])
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.Sequential(
            *[BottleneckTransformer(c_, c_, stride=1, heads=4, mhsa=True, resolution=(dict_w[c2], dict_h[c2]),
                                    expansion=e2) for _ in
              range(n)])
...
class GBS(nn.Module):
    def __init__(self, level, multiplier=0.5, rfb=False, vis=False):
        """
        multiplier should be 1, 0.5
        for small model multiplier = 0.5
        which means, the channel of input can be
        512, 256, 128 -> multiplier=1
        256, 128, 64 -> multiplier=0.5
        For even smaller, change multiplier manually.
        """
        ...
        if level == 0:
            # inter_dim 512,Conv(256,512,3,2)
            self.stride_level_1 = Conv(int(512 * multiplier), self.inter_dim, 3, 2)
            # inter_dim 512,Conv(128,512,3,2)
            self.stride_level_2 = Conv(int(256 * multiplier), self.inter_dim, 3, 2)
            # inter_dim 512,Conv(512,512,3,1)
