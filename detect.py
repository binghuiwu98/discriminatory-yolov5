...
def run(weights='yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if pt:
            ...
            pred_all = model(img, augment=augment, visualize=visualize)
            pred = pred_all[0]
            if vis_cam:
                pred_cam = pred_all[1]
                save_cam_path = os.path.join(save_dir, str(count))
                if not os.path.exists(save_cam_path):
                    os.makedirs(save_cam_path)
                count += 1
                show_CAM(save_cam_path, path, pred_cam, 1,
                         12)  # 12 is classes + 5, according to the shape of prediction
        elif onnx:
...
def show_CAM(save_path, image_path, feature_maps, class_id, all_ids=12, show_one_layer=True):
    """
    ...
    for j in range(3):  # layers
        ...
        for idx, one in enumerate(layer_one_list):
            ...
            if not show_one_layer:
                ...
                cv2.imwrite(save_path + '/head' + str(j) + 'layer' + str(idx) + SHOW_NAME[idx] + ".jpg", show)
